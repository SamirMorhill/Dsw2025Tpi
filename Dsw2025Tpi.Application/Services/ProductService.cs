...
namespace Dsw2025Tpi.Application.Services
{
    public class ProductService
    {
        ...
        public async Task<ProductCreateModel.Response?> GetProductById(Guid Id)
        {
            var product = await _repository.GetById<Product>(Id);
            return new ProductCreateModel.Response(product.Id, product.Sku, product.Name, product.Description, product.CurrentUnitPrice, product.StockQuantity);
        }

        public async Task<ProductCreateModel.Response> ProductUpdate(Guid Id, ProductCreateModel.Request request)
        {
            var product = await _repository.GetById<Product>(Id);
            if (product == null)
            {
            ...
            return new ProductCreateModel.Response(
                updateProduct.Id,
                updateProduct.Sku,
                updateProduct.Name,
